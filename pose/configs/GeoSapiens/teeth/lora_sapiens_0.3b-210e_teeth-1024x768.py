...
optim_wrapper = dict(
    optimizer=dict(
        type='AdamW', lr=2e-4, betas=(0.9, 0.999), weight_decay=0.1),
    paramwise_cfg=dict(
...
# codec settings
codec = dict(
    type='UDPHeatmap', input_size=(image_size[0], image_size[1]),
    heatmap_size=(int(image_size[0] / scale), int(image_size[1] / scale)), sigma=sigma)  ## sigma is 2 for 256
...
model = dict(
    ...
    head=dict(
        ...
        loss=dict(type='GeometricalKeypointMSELoss', geometric_weight=1e-5, use_target_weight=True),
        decoder=codec),
    test_cfg=dict(
        flip_test=True,
        flip_mode='heatmap',
...
dataset_coco = dict(
    ...
    data_prefix=dict(img='imagesTr_clean/'),
)
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        metainfo=dict(from_file='configs/_base_/datasets/teeth.py'),
...
val_evaluator = dict(
    type='EPE',
    ) #ann_file='../data/teeth/annotations/annotations_val.json')
test_evaluator = dict(
    type='EPE',
    ) #ann_file='../data/teeth/annotations/annotations_test.json')