...
###--------------------------------------------------------------
# Please add a comma after each device
# DEVICES=0,
DEVICES=0,1,2,3,
...
RESUME_FROM='../pretrain/checkpoints/sapiens_0.3b/epoch_100.pth'
LOAD_FROM=''
...
###--------------------------------------------------------------
CONFIG_FILE=configs/GeoSapiens/${DATASET}/${MODEL}.py
OUTPUT_DIR="Outputs_EPE/train/${DATASET}/${MODEL}/node"
...
if [ "$mode" = "debug" ]; then
    ...
    CUDA_VISIBLE_DEVICES=${DEVICES} python tools/train.py ${CONFIG_FILE} --work-dir ${OUTPUT_DIR} --no-validate --cfg-options ${OPTIONS} --resume ${RESUME_FROM} 
