...
class LandmarkStatistics(object):
    ...
    def __init__(self, float_format_string=":.3f", missed_landmark_pe=np.nan):
        ...
        self.spacings = {}
        self.distances = {}
        # Add new attributes for storing point-to-point distances
        self.gt_point_distances = {}  # Store groundtruth point-to-point distances
        self.pred_point_distances = {}  # Store predicted point-to-point distances

    def set_groundtruth_and_prediction(
        self,
        predicted_landmarks,
        groundtruth_landmarks,
        spacings,
        normalization_factor=1.0,
        normalization_indizes=None,
    ):
        """
        ...
        for key in predicted_landmarks.keys():
            self.distances[key] = self.get_distances(
                predicted_landmarks[key],
                groundtruth_landmarks[key],
                spacings[key],
                normalization_factor,
                normalization_indizes,
            )
    ...
    def add_landmarks(
        ...
        Args:
            ...
            predicted: The predicted landmarks.
            groundtruth: The groundtruth landmarks.
            spacing: The image spacing.
            normalization_factor: Normalization factor used for distance calculation.
            normalization_indizes: Normalization indizes used for distance calculation.
            gt_dist: Optional pre-computed groundtruth distances. If None, will be calculated.
            pred_dist: Optional pre-computed predicted distances. If None, will be calculated.
        """
        self.predicted_landmarks[image_id] = predicted
        self.groundtruth_landmarks[image_id] = groundtruth
        self.spacings[image_id] = spacing

        # Calculate point-to-point distances if not provided
        if gt_dist is None:
            self.gt_point_distances[image_id] = self.calculate_pairwise_distances(
                groundtruth, spacing
            )
        else:
            self.gt_point_distances[image_id] = gt_dist

        if pred_dist is None:
            self.pred_point_distances[image_id] = self.calculate_pairwise_distances(
                predicted, spacing
            )
        else:
            self.pred_point_distances[image_id] = pred_dist

        # Calculate regular distances between corresponding points
        self.distances[image_id] = self.get_distances(
            predicted, groundtruth, spacing, normalization_factor, normalization_indizes
        )
    ...
    def get_distance(self, prediction, groundtruth, spacing, normalization_factor):
        ...
        if spacing is not None:
            return normalization_factor * np.linalg.norm(
                (groundtruth - prediction) * spacing
            )
        else:
            return normalization_factor * np.linalg.norm(groundtruth - prediction)
    ...
    def get_distances(
        ...
        if normalization_indizes is not None:
            normalization_distance = self.get_distance(
                groundtruth[normalization_indizes[0]],
                groundtruth[normalization_indizes[1]],
                None,
                1.0,
            )
        ...
        return [
            self.get_distance(l0, l1, spacing, normalization_factor)
            for (l0, l1) in zip(predicted, groundtruth)
        ]
    ...
    def get_per_point_pe_statistics(self, landmark_indizes=None):
        """
        Returns mean, stddev and median point errors for each landmark point,
        computed vertically across all images.

        Args:
            landmark_indizes: Optional list of landmark indices to compute statistics for.
                            If None, compute for all landmarks.

        Returns:
            Tuple of (means, stddevs, medians) where each element is a list containing
            the statistic for each landmark point.
        """
        if landmark_indizes is None:
            pe = np.array(list(self.distances.values()))
        else:
        ...
        # Compute statistics along axis 0 (vertically) for each landmark point
        means = np.nanmean(pe, axis=0)
        stddevs = np.nanstd(pe, axis=0)
        medians = np.nanmedian(pe, axis=0)
    ...
    def get_num_correct_id(self, max_distance, landmark_indizes=None):
        ...
        for image_id in self.groundtruth_landmarks.keys():
            correct = self.get_correct_id_per_instance(
                image_id, max_distance, landmark_indizes
            )
            num_correct += len(correct)
        return num_correct

    def get_correct_id_per_instance(
        self, image_id, max_distance, landmark_indizes=None
    ):
        """
        ...
        for i, (groundtruth_landmark, predicted_landmark) in enumerate(
            zip(groundtruth_landmarks, predicted_landmarks)
        ):
            # if not groundtruth_landmark.is_valid or not predicted_landmark.is_valid:
            #     continue
            all_groundtruth_landmarks_and_distances = [
                (
                    other_groundtruth_landmark,
                    self.get_distance(
                        predicted_landmark, other_groundtruth_landmark, spacing, 1.0
                    ),
                )
                for other_groundtruth_landmark in other_groundtruth_landmarks
            ]  #  if other_groundtruth_landmark.is_valid
            if len(all_groundtruth_landmarks_and_distances) == 0:
                continue
            closest_groundtruth_landmark, closest_groundtruth_distance = min(
                all_groundtruth_landmarks_and_distances,
                key=lambda landmark_distance: landmark_distance[1],
            )
            # closest landmark is the groundtruth landmark and the distance is within max_distance
            if (
                closest_groundtruth_landmark == groundtruth_landmark
                and closest_groundtruth_distance <= max_distance
            ):
                correct.append(i)
        return correct
    ...
    def get_num_valid(self, landmark_indizes=None):
        ...
        for image_id in self.groundtruth_landmarks.keys():
            valid_distances, valid_groundtruth, valid_predicted = (
                self.get_valid_per_instance(image_id, landmark_indizes)
            )
    ...
    def get_num_missed_landmarks(self, landmark_indizes=None):
        ...
        num_too_many = 0
        for image_id in self.groundtruth_landmarks.keys():
            missed, too_many = self.get_missed_landmarks_per_instance(
                image_id, landmark_indizes
            )
    ...
    def get_pe_overview_string(self, landmark_indizes=None):
        ...
        :return: Overview string.
        """
        num_valid_distances, num_valid_gt, num_valid_pred = self.get_num_valid(
            landmark_indizes
        )
        stat = self.get_pe_statistics(landmark_indizes)
        overview_string = "PE ({} out of {} valid gt; {} valid pred):\n".format(
            num_valid_distances, num_valid_gt, num_valid_pred
        )
        overview_string += ("mean:   {" + self.float_format_string + "}\n").format(
            stat[0]
        )
    ...
    def get_outliers_pe_string(self, radii):
        ...
        for r, outlier in zip(radii, outliers):
            overview_string += (
                "#outliers >= {}: {} ({" + self.float_format_string + "}%)\n"
            ).format(
                r,
                outlier,
                100 * outlier / num_valid_distances if num_valid_distances > 0 else 0,
            )
        return overview_string
    ...
    def get_per_instance_string(self, outlier_radius, correct_id_max_distance):
        ...
        for image_id in self.groundtruth_landmarks.keys():
            ...
            outliers = [
                ...
                if distance >= outlier_radius
            ]
            missed, too_many = self.get_missed_landmarks_per_instance(image_id)
            correct_id = self.get_correct_id_per_instance(
                image_id, correct_id_max_distance
            )
            _, valid_gt, _ = self.get_valid_per_instance(image_id)
            if (
                len(outliers) > 0
                or len(missed) > 0
                or (len(correct_id) != len(valid_gt))
            ):
                ...
                if len(outliers) > 0:
                    outlier_strings = [
                        ("{} ({" + self.float_format_string + "})").format(i, distance)
                        for i, distance in outliers
                    ]
                    overview_string += (
                        f"outliers > {outlier_radius}: "
                        + ", ".join(outlier_strings)
                        + "\n"
                    )
                if len(valid_gt) - len(correct_id) > 0:
                    not_id_strings = [f"{i}" for i in valid_gt if i not in correct_id]
                    overview_string += (
                        f"not_id > {correct_id_max_distance}: "
                        + ", ".join(not_id_strings)
                        + "\n"
                    )
        return overview_string
    ...
    def get_correct_id_string(self, max_distance, landmark_indizes=None):
        ...
        return (
            "#correct_id <= {}: {} out of {} ({" + self.float_format_string + "}%)\n"
        ).format(
            max_distance,
            num_correct_id,
            num_valid_gt,
            100 * num_correct_id / num_valid_gt if num_valid_gt > 0 else 0,
        )

    def get_per_landmark_string(
        self, landmark_index, outlier_radius, correct_id_max_distance
    ):
        """
        ...
        if outlier_radius is not None:
            num_outliers = self.get_num_outliers(
                [outlier_radius], False, [landmark_index]
            )[0]
            landmark_string += f", o_r{outlier_radius}: {num_outliers: >2}"
        if correct_id_max_distance is not None:
            num_correct_id = self.get_num_correct_id(
                correct_id_max_distance, [landmark_index]
            )
            landmark_string += (
                f", #not_id_r{correct_id_max_distance}: {num_valid_gt - num_correct_id}"
            )
    ...
    def get_landmarks_string(self, outlier_radius, correct_id_max_distance):
        ...
        for i in range(num_landmarks):
            landmarks_strings.append(
                self.get_per_landmark_string(i, outlier_radius, correct_id_max_distance)
            )
        return "\n".join(landmarks_strings) + "\n"

    def get_overview_string(
        self,
        outlier_radii=None,
        per_instance_outliers_radius=None,
        correct_id_distance=None,
    ):
        """
        ...
        if per_instance_outliers_radius is not None or correct_id_distance is not None:
            overview_string += self.get_per_instance_string(
                per_instance_outliers_radius, correct_id_distance
            )
        if correct_id_distance is not None:
            overview_string += self.get_correct_id_string(correct_id_distance)
        overview_string += self.get_landmarks_string(
            outlier_radii[-1], correct_id_distance
        )
        missed, too_many = self.get_num_missed_landmarks()
        overview_string += f"missed landmarks: {missed}; too many: {too_many}\n"
        return overview_string

    def get_per_channel_statistics(self, num_channels, landmark_indizes=None):
        """
        Returns mean, stddev and median point errors for each channel.

        Args:
            num_channels (int): Number of channels/dimensions (typically 2 for 2D or 3 for 3D)
            landmark_indizes (list, optional): Specific landmark indices to calculate statistics for.
                                             If None, use all landmarks.

        Returns:
            list of tuples: List of (mean, stdev, median) for each channel
        """
        if landmark_indizes is None:
            pe = np.array(list(self.distances.values()))
        else:
        ...
        for channel in range(num_channels):
            # Get every num_channels-th element starting from channel index
            channel_pe = pe[..., channel::num_channels].flatten()
            mean = np.nanmean(channel_pe)
            stdev = np.nanstd(channel_pe)
            median = np.nanmedian(channel_pe)
            channel_stats.append((mean, stdev, median))

        return channel_stats

    def get_per_channel_overview_string(self, num_channels, landmark_indizes=None):
        """
        Returns an overview string of the per-channel statistics.
        ...
        Returns:
            str: Overview string containing statistics for each channel
        """
        channel_stats = self.get_per_channel_statistics(num_channels, landmark_indizes)

        overview_string = "Per-Channel Statistics:\n"
        for i, (mean, stdev, median) in enumerate(channel_stats):
            overview_string += f"Channel {i}:\n"
            overview_string += (
                "  mean:   {" + self.float_format_string + "}\n"
            ).format(mean)
            overview_string += (
                "  std:    {" + self.float_format_string + "}\n"
            ).format(stdev)
            overview_string += (
                "  median: {" + self.float_format_string + "}\n"
            ).format(median)

        return overview_string

    def calculate_pairwise_distances(self, points, spacing=None):
        """
        Calculate distances between all pairs of points.

        Args:
            points: List of point coordinates
            spacing: Optional spacing for distance calculation

        Returns:
            numpy array of shape (n_points, n_points) containing pairwise distances
        """
        ...
        for i in range(n_points):
            for j in range(i + 1, n_points):
                if spacing is not None:
                    dist = np.linalg.norm((points[i] - points[j]) * spacing)
                else:
    ...
    def get_mean_pairwise_distances(self):
        """
        Calculate mean and std of pairwise distances across all images for both groundtruth and predictions.

        Returns:
            Tuple of (gt_mean_distances, gt_std_distances, pred_mean_distances, pred_std_distances), 
            each a 2D array of shape (n_points, n_points) containing the mean/std distances between each pair of points
        """
        # Get the number of points from the first image
        n_points = len(next(iter(self.gt_point_distances.values())))

        # Convert dictionaries to 3D arrays (n_images x n_points x n_points)
        gt_distances = np.stack(list(self.gt_point_distances.values()))
        pred_distances = np.stack(list(self.pred_point_distances.values()))

        # Calculate means along first axis (across images)
        gt_mean = np.mean(gt_distances, axis=0)
        pred_mean = np.mean(pred_distances, axis=0)

        # Calculate standard deviations along first axis
        gt_std = np.std(gt_distances, axis=0)
        pred_std = np.std(pred_distances, axis=0)

        return gt_mean, gt_std, pred_mean, pred_std
